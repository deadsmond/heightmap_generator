# source: https://github.com/PaulZC/SRTM_to_STL
...
from math import pi, cos, sin, sqrt, tan, radians
...
def WGS84toOSGB36(lat, lon):
    ...
    VI = nu * F0 * cos(lat) ** 5 * (
            5 - 18 * tan(lat) ** 2 + tan(lat) ** 4 + 14 * eta2 - 58 * eta2 * tan(lat) ** 2) / 120
...
def from_latlon(latitude, longitude, force_zone):
    ...
    if not -180.0 <= longitude <= 180.0:
        raise ValueError('northing out of range (must be between 180 deg W and 180 deg E)')
...
def convert_srtm_to_csv(filename: str, UK: bool = False):
    try:
        ...
        if UK:  # Force a UTM zone for non-UK data?
            if force_zone == '':
                force_zone = input('If you want to force a UTM zone, enter it now: ')
            if force_zone != '':
                try:
        ...
        just_filename = os.path.splitext(just_filename)[0]

        start_lon = float(just_filename[-3:])  # Get the start latitude from the filename
        if just_filename[3] == 'W': start_lon = -start_lon
        start_lat = float(just_filename[1:3])  # Get the start longitude from the filename
        if just_filename[0] == 'S': start_lat = -start_lat
        ...
        # Include "width,height,hgt_min,hgt_max" as the first line
        fp = open(outfile, 'w')
